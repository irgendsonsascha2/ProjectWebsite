...
**Optionales TOTP-2FA (klassische Site):** Einrichtung unter **`index.php?page=account`** (nur eingeloggt). Nach Passwort-Login leitet **`bridge_auth.php`** bei aktivem 2FA auf **`index.php?page=login&step=2fa`**; die zweite Phase läuft über **`bridge_auth_2fa.php`**. Backup-Codes werden gehasht in Mongo gespeichert und sind einmalig nutzbar. Felder: `two_factor_*` auf `users` (optionaler Index: `dbScripts/11_db_init_users_two_factor.php`).
